...
class BoardTest < Minitest::Test
  ...
  def test_ships_cannot_be_placed_diagonally
    ...
    assert_equal false, @board.valid_placement?(@ship2,["A2", "B3", "C4"])
    assert_equal false, @board.valid_placement?(@ship, ["A1", "B2"])
  end

  def test_ships_can_only_be_placed_in_consecutive_coordinates

    assert @board.valid_placement?(@ship2, ["B1", "C1", "D1"])
